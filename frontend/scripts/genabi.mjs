...
const CONTRACT_NAME = "EncryptedOneTimeCode";

// <root>/pro13
const rel = "..";

// <root>/pro13/frontend/abi
...
function readDeployment(chainName, chainId, contractName, optional) {
  ...
  if (!fs.existsSync(chainDeploymentDir)) {
    // In CI/Vercel, deployments might not exist - use optional mode
    if (process.env.CI || process.env.VERCEL) {
      console.warn(
        `${line}Deployment directory '${chainDeploymentDir}' not found in CI/Vercel environment. Using fallback.${line}`
      );
      if (!optional) {
        return undefined; // Return undefined instead of exiting in CI
      }
      return undefined;
    }
    console.error(
...
if (!deploySepolia) {
  if (deployLocalhost) {
    ...
    // In CI/Vercel, if no deployments exist, try to use existing ABI file
    const existingAbiPath = path.join(outdir, `${CONTRACT_NAME}ABI.ts`);
    if (fs.existsSync(existingAbiPath)) {
      console.log(`Using existing ABI file: ${existingAbiPath}`);
      // Read the existing ABI to extract it
      const existingAbiContent = fs.readFileSync(existingAbiPath, "utf-8");
      const abiMatch = existingAbiContent.match(/export const \w+ABI = ({[\s\S]*?}) as const;/);
      if (abiMatch) {
        const abiObj = JSON.parse(abiMatch[1]);
        deployLocalhost = { abi: abiObj.abi, address: "0x0000000000000000000000000000000000000000" };
        deploySepolia = { abi: abiObj.abi, address: "0x0000000000000000000000000000000000000000" };
      }
    } else {
      console.error(`${line}No deployment found and no existing ABI file. Cannot generate ABI.${line}`);
      process.exit(1);
    }
  }
}
