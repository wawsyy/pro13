  ...
  public load(): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      script.onerror = (error) => {
        ...
        console.error(`[RelayerSDKLoader] Failed to load from: ${SDK_CDN_URL}`);
        reject(
          new Error(
            `RelayerSDKLoader: Failed to load Relayer SDK from ${SDK_CDN_URL}. This may be a network issue or the CDN is unreachable.`
          )
        );
      };
