...
async function getChainId(
  ...
  // Check if provider has request method (EIP-1193 provider)
  ...
  throw new Error("Unable to get chainId from provider");
}
...
async function getWeb3Client(rpcUrl: string) {
  ...
  try {
    ...
    console.error(`[FHEVM] Failed to connect to ${rpcUrl}:`, e);
    throwFhevmError(
...
async function getFHEVMRelayerMetadata(rpcUrl: string) {
  ...
  try {
    ...
    console.error(`[FHEVM] Failed to get relayer metadata from ${rpcUrl}:`, e);
    // Don't throw error here, just return undefined to allow fallback
    return undefined;
  } finally {
    rpc.destroy();
  }
...
export const createFhevmInstance = async (parameters: {
  ...
  if (isMock) {
    ...
    console.log(`[FHEVM] Detected mock chain (chainId: ${chainId}), attempting to use mock mode with ${rpcUrl}`);
    ...
    if (fhevmRelayerMetadata) {
      ...
      console.log(`[FHEVM] Using mock mode with metadata:`, fhevmRelayerMetadata);
      notify("creating");
      ...
      console.warn(`[FHEVM] Mock chain detected but could not fetch metadata from ${rpcUrl}. Make sure Hardhat node is running with FHEVM support.`);
    }
  }
